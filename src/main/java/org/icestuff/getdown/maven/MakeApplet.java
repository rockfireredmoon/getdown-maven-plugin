...
public class MakeApplet extends AbstractGetdownMojo {
	...
	Artifact getdown = (Artifact) plugin.getArtifactMap().get(
	...
	if (sign != null) {
	    ...
	    File signedJar = new File(workDirectory, unsignedJar.getName()
		    + ".signed");
	    Util.deleteFile(signedJar);
	    verboseLog("Sign " + unsignedJar.getName());
	    signTool.sign(sign, unsignedJar, signedJar);
	    getLog().debug(
		    "lastModified signedJar:" + signedJar.lastModified()
			    + " unprocessed signed Jar:"
