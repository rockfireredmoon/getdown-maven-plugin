...
public class MakeUpdatesMojo extends AbstractGetdownMojo {
	...
	try {
		...
		throw new MojoExecutionException(
			"didn't find artifact with main class: " + mainClass
				+ ". Did you specify it? ");
	    }
	...
	PrintWriter writer = new PrintWriter(new File(workDirectory,
		"getdown.txt"));
	try {
	...
	if (ui.progressBar != null) {
	    writer.println(String
		    .format("ui.progress_bar = %s", ui.progressBar));
	}
	if (ui.progressText != null) {
	    writer.println(String.format("ui.progress_text = %s",
		    ui.progressText));
	}
	if (ui.status != null) {
	...
	if (ui.installError != null) {
	    writer.println(String.format("ui.install_error = %s",
		    ui.installError));
	}
    }
	...
	if (dependencies.getIncludes() != null
	...
	if (dependencies.getExcludes() != null
	    ...
	    failed = checkDependencies(dependencies.getExcludes(), artifacts)
		    || failed;
	}
	...
	if (dependencies != null && dependencies.getIncludes() != null
		&& !dependencies.getIncludes().isEmpty()) {
	    filter.add(new IncludesArtifactFilter(dependencies.getIncludes()));
	}
	if (dependencies != null && dependencies.getExcludes() != null
		&& !dependencies.getExcludes().isEmpty()) {
	    filter.add(new ExcludesArtifactFilter(dependencies.getExcludes()));
	}

	Collection<Artifact> artifacts = excludeTransitive ? project
		.getDependencyArtifacts() : project.getArtifacts();
	...
	if (!Artifact.SCOPE_SYSTEM.equals(artifact.getScope())
		...
		if (toCopy == null) {
		    ...
		    getLog().error(
			    "artifact repository: " + artifact.getRepository());
		    getLog().error(
			    "artifact repository: " + artifact.getVersion());
		    throw new IllegalStateException("artifact " + artifact
			    + " has no matching file, why? Check the logs...");
		}

		String name = getDependencyFileBasename(artifact,
			outputJarVersions);

		boolean copied = copyJarAsUnprocessedToDirectoryIfNecessary(
			toCopy, getLibDirectory(), name + ".jar");
		...
		if (mainClass != null) {
		    ...
		    boolean containsMainClass = artifactContainsClass(artifact,
			    mainClass);
	    ...
	    verboseLog("Skipping artifact of scope " + artifact.getScope()
		    + " for " + getLibDirectory().getName());
	}
    }

    public String getDependencyFileBasename(Artifact artifact,
    ...
    protected boolean copyJarAsUnprocessedToDirectoryIfNecessary(
	    File sourceFile, File targetDirectory, String targetFilename)
	    throws MojoExecutionException {
	...
	if (shouldCopy) {
	    ...
	    getLog().debug(
		    "Source file hasn't changed. Do not reprocess "
     ...
     */
    private boolean checkDependencies(List<String> patterns,
	...
	for (String pattern : patterns) {
	    failed = ensurePatternMatchesAtLeastOneArtifact(pattern, artifacts)
		    || failed;
	}
	return failed;
    }
     ...
     */
    private boolean ensurePatternMatchesAtLeastOneArtifact(String pattern,
	    Collection<Artifact> artifacts) {
	...
	for (Artifact artifact : artifacts) {
	    getLog().debug(
		    "checking pattern: " + pattern + " against " + artifact);
	...
	if (noMatch) {
	    getLog().error(
		    "pattern: " + pattern + " doesn't match any artifact.");
	}
	return noMatch;
    }
    ...
    private List<String> copyResourceSets(ResourceSet[] resourceSets)
	    throws MojoExecutionException {
	...
	for (ResourceSet s : resourceSets) {
		...
		if (!sourceDirectory.isDirectory()) {
		    ...
		    getLog().debug(
			    "Copying resource set from "
				    + sourceDirectory.getAbsolutePath());

		    String include = s.includes == null ? "**" : Util.concat(
			    s.includes, ", ");
		    String excludes = s.excludes == null ? Util.concat(
			    DirectoryScanner.DEFAULTEXCLUDES, ", ")
			    : (Util.concat(DirectoryScanner.DEFAULTEXCLUDES,
				    ", ") + Util.concat(s.excludes, ", "));

		    paths.addAll(Util.copyDirectoryStructure(getLog(),
			    sourceDirectory, workDirectory, include, excludes));
		}
    ...
    public boolean artifactContainsClass(Artifact artifact,
	    final String mainClass) throws MojoExecutionException {
	...
	try {
	    ...
	    throw new MojoExecutionException("Could not get artifact url: "
		    + artifact.getFile(), e);
	}
	...
	try {
	    ...
	    getLog().debug(
		    "artifact " + artifact
			    + " doesn't contain the main class: " + mainClass);
	    containsClass = false;
	} catch (Throwable t) {
	    getLog().info(
		    "artifact "
